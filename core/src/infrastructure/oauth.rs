...
///
/// # Errors
/// Returns an error if the loopback listener cannot be bound, the browser
/// cannot be opened, the flow times out or the provider reports an
/// authorization error, the returned state does not match (possible CSRF),
/// or the authorization-code token exchange fails.
pub async fn run_loopback_pkce(
    ...
    let task = tokio::task::spawn_blocking(move || wait_for_callback(&listener));
...
/// Block until the loopback listener receives the OAuth `/callback` request
/// (or the flow times out), answering the browser along the way. Returns the
/// authorization `code` and returned `state`.
fn wait_for_callback(listener: &tiny_http::Server) -> Result<(String, String)> {
