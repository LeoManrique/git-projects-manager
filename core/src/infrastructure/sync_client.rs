...
impl SyncClient {
    ...
    /// # Errors
    /// Returns an error if the sign-in request fails, the server rejects it,
    ...
    /// # Errors
    /// Returns an error if the sign-out request fails or the server rejects
    /// it with a status other than 401 (already signed out).
